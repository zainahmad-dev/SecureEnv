...
| `NEXT_PUBLIC_GITHUB_URL` / `NEXT_PUBLIC_PORTFOLIO_URL` | Optional. The outbound links on the landing page. GitHub defaults to this repository; the portfolio link isn't rendered at all until set, since a guessed URL on a page opened from a CV is worse than a missing one | Public by nature — their purpose is to be clicked |
